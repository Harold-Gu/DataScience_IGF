...
from . import crawl
from .config import (ARCHIVED, DASHBOARD, DETAIL_RE, IGF_BASE, PARTICIPANTS,
                     SESSION_TYPES, STEPS, WORKERS, YEAR_END, YEAR_START,
                     _FILE_MAP, _MANIFEST, _REPORT_HINTS, _failed_lock,
                     _failed_seen, _fetch_err, _stats, _stats_lock, year_range)
...
def step_sessions(out_root,workers=WORKERS,years=None,limit=None):
            ...
            for ti,tmpl in enumerate(templates):
                ...
                while pi<len(list_pages)and len(list_pages)<200:
                    ...
                    for a in soup.find_all("a",href=True):
                        href=crawl._unwrap_wb(a.get("href",""))
                        if not href or href.startswith("#"):continue
                        full=crawl._make_url(href)
                    ...
                    for nxt in crawl._next_page_links(soup,cur):
                        ...
                        r2=crawl._fetch(nxt,wb_year=y)or crawl._fetch_wb(nxt,y)
                        if r2 is None:
            ...
            if total_links>0:
                ...
                for label,links_list in[("proposals",links_proposals),("content",links_content)]:
                    ...
                    for link in links_list:
                        ...
                        fpath=os.path.join(sub,f"{crawl._clean(name)}.html")
                        tasks.append((link,fpath,tag))
                if limit and total_downloaded>=limit:
                    continue
                if limit:
                    tasks=tasks[:max(0,limit-total_downloaded)]
                if tasks:
                    crawl._download_batch(tasks,workers)
                    total_downloaded+=len(tasks)
    ...
    if grand_total==0:print("  No session links found.");return
    print(f"\n  Sessions done: {total_downloaded} pages downloaded.")
    ...
    for y in year_range(years):
        ...
        for url_tmpl in[IGF_BASE+"/en/content/igf-{year}-schedule",IGF_BASE+"/en/igf-{year}-schedule"]:
            ...
            if r is not None:
                ...
                crawl._deep_crawl_parallel(test_url,sub,workers)
    ...
    for y in year_range(years):
        ...
        while True:
            ...
            if page==0 and os.path.exists(os.path.join(sub,"index.html")):
                ...
                r=crawl._fetch(url)or crawl._fetch_wb(url,y)
            ...
            if not os.path.exists(ppath):crawl._atomic_write_text(ppath,html)
            ...
            for a in (main.find_all("a",href=True) if not is_dashboard_spa else []):
                ...
                full=crawl._make_url(href,seed_url)

                if crawl._is_file(full):

                    fp=os.path.join(sub,crawl._clean(full.split("/")[-1].split("?")[0]))

                    page_tasks.append((full,fp,str(y)))

                elif crawl._same_domain(full,IGF_BASE)and"/content/"in full:

                    fp=os.path.join(sub,f"{crawl._clean(full.split('/')[-1].split('?')[0])}.html")

                    page_tasks.append((full,fp,str(y)))

            if page_tasks:crawl._download_batch(page_tasks,workers);total_items+=len(page_tasks)
...
def step_archived_dashboard(out_root,years=None,workers=WORKERS):
    ...
    for y,path in ARCHIVED.items():
        ...
        crawl._deep_crawl_parallel(IGF_BASE+path,sub,workers)
...
def run(args):
    ...
    if args.retry_failed:
        ...
        crawl._set_failed_log(os.path.join(retry_dir, "failed_urls_retry.tsv"))
    ...
    print(f"  Steps: {', '.join(sorted(do))} | Workers: {args.workers} | "

          f"Years: {YEAR_START}-{YEAR_END} | Output: {os.path.abspath(out)}")
    ...
    crawl._set_failed_log(os.path.join(out, "failed_urls.tsv"))
    ...
    if "archived" in do or "dashboard" in do:
        ...
        step_archived_dashboard(out, set(years) if years else None, args.workers)

        crawl._step_note("archived_dashboard", s0, ts)
        ...
        if "reports" in do:
            ...
            _discover_reports(out, args.workers, years=years)

            crawl._step_note("reports_discovery", s0, ts)
    ...
    if "participants" in do:
        ...
        step_participants(out, args.workers)

        crawl._step_note("participants", s0, ts)
