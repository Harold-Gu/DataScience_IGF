"""
IGF Advanced Trend Visualization (Refactored)
Generates 3 advanced plots:
  1. Top 6 themes evolution over time
  2. Topic shifts across top 4 organizations (4 subplots)
  3. Core speaker-organization relationship scatter plot
...
def visualize_advanced_trends(csv_path: str):
    ...
    # ---- Plot 2: Topic shifts across top 4 organizations ----
    org_theme_df = (
        df[["Year", "Organizations_List", "Themes_List"]]
        .explode("Organizations_List").reset_index(drop=True)
        .explode("Themes_List").reset_index(drop=True)
    )
    org_theme_df = org_theme_df[
        (org_theme_df["Organizations_List"] != "") & (org_theme_df["Themes_List"] != "")
    ].dropna()
    ...
    for i, org in enumerate(top_4_orgs):
        ...
        axes[i].set_xticks(list(all_years))
    ...
    # ---- Plot 3: Speaker-organization relationship scatter ----
    spk_org_df = (
        df[["Year", "Speakers_List", "Organizations_List"]]
        .explode("Speakers_List").reset_index(drop=True)
        .explode("Organizations_List").reset_index(drop=True)
    )
    spk_org_df = spk_org_df[
        (spk_org_df["Speakers_List"] != "") & (spk_org_df["Organizations_List"] != "")
    ].dropna()
    ...
    sns.scatterplot(
        data=primary_orgs, x="Year", y="Speakers_List",
        hue="Organizations_List", s=250, marker="s",
        palette="tab20", edgecolor="black", linewidth=0.5,
    )
